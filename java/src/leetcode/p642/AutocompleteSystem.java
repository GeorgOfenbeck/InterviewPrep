...
class AutocompleteSystem {
    ...
    public static void main(String[] args) {
        ...
        int[] f = {12,9,4,4,1,5,3,4,7,9,2,4,2,3,11,13,1,3,4,10,7,1,9,5,10,14,5,3,2,11,5,14,4,13,11,5,15,8,1,12,2,11,4,2,11,14,9,12,1,7,13,11,7,2,6,10};
        AutocompleteSystem auto = new AutocompleteSystem(s, f);
        //auto.dictonary.addString("i a");
        //auto.dictonary.addString("i a");
        auto.input('w');
        auto.input('o');
    ...
    /*
    class Top3Strings implements Comparable<Top3Strings>{
        Integer frequency;
        String s;

        Top3Strings(Integer frequency, String s){
        ...
        @Override
        public int compareTo(Top3Strings o) {
            int cmp = o.frequency.compareTo(this.frequency);
            if (cmp == 0){
    ...
    */
    class Top3 implements Comparable<Top3> {
        Integer frequency = 0;
        TrieNode endSymbol;
        ...
        public boolean equals(Object o) {
            ...
            Top3 top3 = (Top3) o;
            return Objects.equals(frequency, top3.frequency) &&
                    Objects.equals(endSymbol, top3.endSymbol);
        }

        @Override
        public int hashCode() {
            return Objects.hash(frequency, endSymbol);
        }

        Top3(Integer frequency, TrieNode endSymbol) {
            this.frequency = frequency;
            this.endSymbol = endSymbol;
        }


        private String TrieNode2String(TrieNode end) {
            TrieNode ptr = end;
        ...
        public int compareTo(Top3 o) {
            ...
            if (cmp != 0) return cmp;
                ...
                String a = TrieNode2String(this.endSymbol);
                String b = TrieNode2String(o.endSymbol);
    ...
    class TrieNode {
        ...
        Character c;
        Integer frequency = 0;
        PriorityQueue<Top3> top3 = new PriorityQueue<>();

        TrieNode(Character c) {
            this.c = c;
        }

        void addString(String s, Integer frequency) {
            TrieNode ptr = this;

            for (Character c : s.toCharArray()) {
                ptr.children.putIfAbsent(c, new TrieNode(c));
                TrieNode child = ptr.children.get(c);
                child.parent = ptr;
                ptr = child;
            }
            //at this point we put the whole string

            ptr.frequency = frequency;

            TrieNode endNode = ptr;
            while (ptr != null) {
                ptr.top3.add(new Top3(frequency, endNode));
                if (ptr.top3.size() > 3)
                    ptr.top3.poll(); //drop the smallest
                ptr = ptr.parent;
            }
        }
        ...
        void addString(String s) {
            ...
            TrieNode endNode = ptr;
            while (ptr != null) {
                ptr.top3.remove(new Top3(frequency - 1, endNode));
                ptr.top3.add(new Top3(frequency, endNode));
                if (ptr.top3.size() > 3)
                    ptr.top3.poll(); //drop the smallest
                ptr = ptr.parent;
    ...
    public AutocompleteSystem(String[] sentences, int[] times) {
        dictonary = new TrieNode('$');
        cur = dictonary;
        input = new StringBuffer();
        for (int i = 0; i < sentences.length; i++) {
            dictonary.addString(sentences[i], times[i]);
        }
    }

    public List<String> input(char c) {
        if (c == '#') {
            String newWord = input.toString();
            dictonary.addString(newWord);
            input = new StringBuffer();
            cur = dictonary;
            ...
            if (cur.children.containsKey(c)) {
                ...
                cur = node;
                return top3ToLinkedList(node.top3);
            } else {
                cur = new TrieNode('$');
                return new LinkedList<>();
            }
    ...
    LinkedList top3ToLinkedList(PriorityQueue<Top3> top3) {
        Stack<Top3> store = new Stack();
        LinkedList<String> res = new LinkedList<>();
        //TreeSet<Top3Strings> sort = new TreeSet<>();
        while (!top3.isEmpty()) {
            Top3 ele = top3.poll();
            if (ele != null) {
                store.push(ele);
                TrieNode ptr = ele.endSymbol;
                StringBuffer sb = new StringBuffer();
                while (ptr.parent != null) {
                    sb.append(ptr.c);
                    ptr = ptr.parent;
                }
                res.addFirst(sb.reverse().toString());
                //      sort.add(new Top3Strings(ele.frequency,sb.reverse().toString()));
            }
        }
        while (!store.isEmpty()) top3.add(store.pop());
        //while (!sort.isEmpty()) res.add(sort.pollFirst().s);
        return res;
    }
