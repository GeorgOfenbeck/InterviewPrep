package leetcode.p816;

import java.lang.reflect.Array;
import java.util.ArrayList;
import java.util.List;

public class Solution {

    /*
    We had some 2-dimensional coordinates, like "(1, 3)" or "(2, 0.5)".  Then, we removed all commas, decimal points, and spaces, and ended up with the string S.  Return a list of strings representing all possibilities for what our original coordinates could have been.

Our original representation never had extraneous zeroes, so we never started with numbers like "00", "0.0", "0.00", "1.0", "001", "00.01", or any other number that can be represented with less digits.
 Also, a decimal point within a number never occurs without at least one digit occuring before it, so we never started with numbers like ".1".

The final answer list can be returned in any order.  Also note that all coordinates in the final answer have exactly one space between them (occurring after the comma.)

Example 1:
Input: "(123)"
Output: ["(1, 23)", "(12, 3)", "(1.2, 3)", "(1, 2.3)"]
Example 2:
Input: "(00011)"
Output:  ["(0.001, 1)", "(0, 0.011)"]
Explanation:
0.0, 00, 0001 or 00.01 are not allowed.
Example 3:
Input: "(0123)"
Output: ["(0, 123)", "(0, 12.3)", "(0, 1.23)", "(0.1, 23)", "(0.1, 2.3)", "(0.12, 3)"]
Example 4:
Input: "(100)"
Output: [(10, 0)]
Explanation:
1.0 is not allowed.


Note:

4 <= S.length <= 12.
S[0] = "(", S[S.length - 1] = ")", and the other elements in S are digits.


     */


    public static void main(String[] args) {
        Solution sol = new Solution();
        sol.ambiguousCoordinates("(123)");
    }


    class Pair {
        String a;
        String b;

        Pair(String a, String b) {
            this.a = a;
            this.b = b;
        }
    }

    public Pair splitAt(String S, int idx) {
        if (S.isEmpty())
            return new Pair("", "");
        if (S.length() == 1)
            return new Pair(S, "");


        String a = S.substring(0, idx);
        String b = S.substring(idx, S.length());
        return new Pair(a, b);
    }

    public ArrayList<Pair> splitthem(String S) {
        ArrayList<Pair> ret = new ArrayList<>();
        for (int i = 1; i < S.length(); i++) {
            Pair pair = splitAt(S, i);
            ret.add(pair);
        }
        return ret;
    }

    public ArrayList<String> variants(String S) {
        ArrayList<String> str = new ArrayList<String>();
        if (!S.startsWith("0") || S.length() == 1)
            str.add(S);
        if (S.length() == 1) {
            return str;
        }

        for (int i = 1; i < S.length(); i++) {
            Pair pair = splitAt(S, i);
            String a = pair.a;
            String b = pair.b;
            if (!(a.startsWith("0") && a.length() > 1) && !b.endsWith("0")) {
                str.add("" + a + "." + b + "");
                //str.add("("+a+", "+b+")");
            }
        }
        return str;
    }

    public List<String> ambiguousCoordinates(String S) {
        String withoutbrackets = S.substring(1, S.length() - 1);
        ArrayList<Pair> pairs = splitthem(withoutbrackets);
        ArrayList<String> result = new ArrayList<>();
        for (int j = 0; j < pairs.size(); j++) {
            Pair cur = pairs.get(j);
            String a = cur.a;
            String b = cur.b;

            ArrayList<String> avar = variants(a);
            ArrayList<String> bvar = variants(b);

            for (int x = 0; x < avar.size(); x++)
                for (int y = 0; y < bvar.size(); y++) {
                    result.add("(" + avar.get(x) + ", " + bvar.get(y) + ")");
                }

        }
        return result;
    }
}