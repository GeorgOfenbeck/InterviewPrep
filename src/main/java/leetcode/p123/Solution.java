...
class Result{
    ...
    void addSetandResult(Result other, Profit px){
        ...
        if (!x)
            System.out.println("?");
        last = px;
        if (sort.size() > this.k || !sort.contains(last))
            System.out.println("?");
    }
...
class Solution {
    ...
    public int maxProfit(int[] prices, int k) {
        ...
        if (profits.isEmpty()) return 0;
            ...
            return results.get(results.size()-1).profit();
    ...
    private void updateFront(ArrayList<Result> ks, Profit px){
        for (Result r : ks){
            r.backup(); //make sure we have the old ones
        }
        ...
        for (int i = 0; i < ks.size(); i++){
            ...
            if (i > 0){
                prev = ks.get(i-1);
                profitPrevAndPx = prev.oldprofit() + px.profit;
            } else {
                profitPrevAndPx = Integer.MIN_VALUE;
            }
